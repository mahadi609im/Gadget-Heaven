...
import { removeFromWishlist, setStoreData } from '../../../Utility/StoreData';
import { use } from 'react';
import { HandleWishlistBtnContext } from '../Dashboard';

const Wishlist = ({ wishlistItem }) => {
  const { product_id, product_title, product_image, description, price } =
    wishlistItem;

  const handleWishlistRemove = use(HandleWishlistBtnContext);
    ...
    <div className="p-8 bg-white rounded-2xl flex gap-8 items-center mb-6">
      ...
      <div className="max-w-[909px] w-full">
        <div className="flex justify-between items-center">
          ...
          <button
            onClick={() => handleWishlistRemove(product_id)}
        ...
        <button
          ...
          className={`py-[13px] px-[26px] rounded-4xl text-white mt-4 cursor-pointer bg-[#9538E2]`}
        >
          Add to Card
        </button>
      </div>
